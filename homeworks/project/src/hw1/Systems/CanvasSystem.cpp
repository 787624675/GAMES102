...
constexpr int sample_num = 500;           // it decides the step to draw points of a line
constexpr float base_tangent_len = 50.f;      // The initial length of the tangent line
constexpr float t_step = 1.f / (sample_num - 1);  // The step to draw points of a line
constexpr float point_radius = 3.f;              // The radius of the points of a line
constexpr ImU32 line_col = IM_COL32(39, 117, 182, 255);       // The color of a line
...
// This is a function pointer, it can be used like:  ParamFunc[0](vector1, vector2)
...
// Make this function inline because it will be called thousands of times at once
...
void draw(std::vector<Ubpa::pointf2>& points,
	std::vector<Ubpa::pointf2>& ltangent,     // Why tangent?
	std::vector<Ubpa::pointf2>& rtangent,
	CanvasData*, ImDrawList*, const ImVec2&,   // Why passed CanvasData and ImDrawList? What's ImVec2?
	int edit_flag = 0, bool with_slope = false);   // Different cases to draw are marked by booleans
void drawWithBezier(std::vector<Ubpa::pointf2>& points,
	std::vector<Ubpa::pointf2>& ltangent,          // Another draw function
	std::vector<Ubpa::pointf2>& rtangent,
	CanvasData*, ImDrawList*, const ImVec2&,
	int edit_flag = 0);
// Note that the variable of points and slope are all refferent pass
...
void CanvasSystem::OnUpdate(Ubpa::UECS::Schedule& schedule) {
	schedule.RegisterCommand([](Ubpa::UECS::World* w) {
		...
		if (ImGui::Begin("Canvas")) {
			...
			// Two cases to switch the type of fitting
			...
			if (is_hovered) {
				if (ImGui::IsMouseClicked(ImGuiMouseButton_Middle) && data->edit_point == 0) {
					// Middle mouse is clicked, stop adding points
					if (data->points.size() > 0) data->pop_back();   // Pop back the changing point
					// spdlog::info("d:{}", data->points.size());
					data->enable_add_point = false;
					data->adding_last_point = false;
				}
				if (data->enable_add_point) {   // Turn on this through right click menu
					change_flag = true;         
					if (ImGui::IsMouseClicked(ImGuiMouseButton_Left)) {
						data->push_back(mouse_pos_in_canvas);       // A highly encapsulation function
					}
					else {
			...
			// enable edit point(including moving tangent and point)
			if (data->edit_point != 0) {  
				...
				if (data->editing_index != -1) {
					if (data->enable_move_point) {
						...
						if (data->fitting_type == 0) { // Cubic spline
							draw(temp_p, data->ltangent, data->rtangent, data, draw_list, origin, 1);
						}
						else if (data->fitting_type == 1) {  // Cubic Bezier
				...
				// G0
				if (data->edit_point == 2) {  // Cubic Bezier
					if (!data->enable_move_tan) {  // Turn on through right click menu
						data->editing_tan_index = 0;  // Initialize
						for (int i = 0; i < data->points.size(); i++) {  // redraw
							if (std::abs(data->ltangent[i][0] - mouse_pos_in_canvas[0]) < point_radius
								...
								data->editing_tan_index = -(i + 1);         // locate index every moment
								break;
							}
							// ltangent and rtangent are used to record the terminal points of the tangent lines
							// Why -(i+1) and i+1 ? —— left tangent line and right tangent line
					...
					if (data->editing_tan_index < 0) {
						...
						if (data->enable_move_tan) {
							...
							// draw the line at every moment
							// Drawing a line will need points, left-tangent, right tangent
							draw(data->points, temp_lt, data->rtangent, data, draw_list, origin, 2, true);
							// Scan whather left mouse is clicked
							if (ImGui::IsMouseClicked(ImGuiMouseButton_Left)) {
								// change ltangent in real time
				...
				if (data->edit_point == 3) {
					...
					if (data->editing_tan_index < 0) {
						...
						if (data->enable_move_tan) {
							...
							// This is how to change right tangent
							// Caculate the right tangent point
							...
							// change right tangent when moving left tangent
							if (ImGui::IsMouseClicked(ImGuiMouseButton_Left)) {
								data->ltangent[-1 - data->editing_tan_index] = mouse_pos_in_canvas;
